...
//    destinationType = context.get("destinationtype", String.class);
    ...
    initialContextFactory = context.getString("initialcontextfactory");
    brokerUrl = context.getString("brokerurl");
    destinationName = context.getString("destination");
    destinationType = context.getString("destinationtype");
  ...
  private void destroyConnection() {        logger.info(" JMS sink {}  destroy connection --", this.getName()); //
    try {
      if (jmsSession != null) {
        jmsSession.close();
...
*/
  ...
  public Status process() throws EventDeliveryException {
    ...
    try {
      ...
      ////////////
      int txnEventCount = 0;
      for( txnEventCount = 0; txnEventCount < 100; txnEventCount++ ) {
  ...
  private void sendEvent(Event event) throws JMSException {  logger.info("JMS sink {} send Event", this.getName());//
